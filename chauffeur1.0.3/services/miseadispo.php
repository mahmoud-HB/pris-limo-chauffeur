...
if (! isset ( $_REQUEST ['departMise'] )) {
	if ((! isset ( $_SESSION ['departMise'] )) || (empty ( $_SESSION ['departMise'] )))
		header ( 'Location: ../index.php' );
	$marLieux = $_SESSION ['departMise'];
} else {
	$marLieux = $_REQUEST ['departMise'];
	$_SESSION ['departMise'] = $marLieux;
}
...
try {
	if (isset($marLieux)) {
		if (!empty($marLieux)){
			...
			$dist = getDistance("75001 Paris, France", $marLieux);
			...
			if ($dist == "0"){
				...
				$dist = round( $dist ['distance'] );
			}
			if (isset($code)){
				$prix = getPrixMiseADispo($dist,$vehicle,$heure,$date,$code,$nbHours);
			}else{
				$prix = getPrixMiseADispo($dist,$vehicle,$heure,$date,"",$nbHours);
				}
			}
	}else{
		throw new Exception ( 'Une erreur est survenue. Merci de contacter l\'administrateur du site. (Redirection vers la page d\'acceuil dans 5 sec ...)');
					...
					<div class="col span_2_of_3">
						<div style="margin-left: 30px;">
							...
							<ul class="b">
								...
								<li>Départ de :&emsp; <?php echo $marLieux; ?></li>
